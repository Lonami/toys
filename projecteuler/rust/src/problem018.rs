...
fn solve() {
    ...
    // Work backwards, always adding the maximum
    for i in (0..triangle.len() - 1).rev() {
        for j in 0..triangle[i].len() {
            let bottom_left = triangle[i + 1][j + 0];
            let bottom_right = triangle[i + 1][j + 1];
            triangle[i][j] += cmp::max(bottom_left, bottom_right);
        }
    }
